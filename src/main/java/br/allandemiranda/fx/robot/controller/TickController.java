...
public class TickController {
  ...
  public Mono<TickDto> create(@PathVariable @Pattern(regexp = "^[A-Z]{6}$") @Valid String name, @RequestBody @Valid TickCreateDto tickCreateDto) {
    ...
    return SymbolUtils.getSymbol(name, this.getSymbolService()).flatMap(symbolDto -> this.getTickService().create(symbolDto, tickCreateDto))
        .doOnError(throwable -> log.warn("Trouble for creating tick [name={}, tickCreateDto={}]", name, tickCreateDto, throwable))
