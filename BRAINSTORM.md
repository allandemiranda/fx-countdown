...
- For the comment, needs contains the probability for each operation (buy, sell, neutral) generate by the XGBoost4j, plus the time in seconds to the system generate the answer, plus the uuid of the dashboard.
...
- DELETE HTTP "api/dashboards/{uuid}"
    - When you want to delete all data from a specificities dashboard;
    - Is a cascade mode from dashboards to indicators, deleting all indicators data linked to this dashboard;
    - Delete the XGBoost files and the DMatrix file that have the uuid from this dashboard;
...
Remember that this a PATCH operation, no one of the objects on the body are required, but until they have, should be updated on the database, and returned the full dashboard object. At the end of the update of the data, run the
